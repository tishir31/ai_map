...
begin
  ...
  select r.id into rid from public.research_runs r where r.public_mode and (p_run_id is null or r.id=p_run_id)
    and exists (select 1 from public.research_tasks q where q.run_id=r.id and q.status='pending' and q.public_attempts<3)
    and not exists (select 1 from public.research_tasks q where q.run_id=r.id and q.status='running' and q.locked_at>now()-interval '90 seconds')
  ...
  update public.research_tasks set status='pending',locked_at=null,locked_by=null where run_id=rid and status='running' and locked_at<=now()-interval '90 seconds' and public_attempts<3;
  select * into t from public.research_tasks where run_id=rid and status='pending' and public_attempts<3 order by created_at,id limit 1 for update;
...
declare t public.research_tasks; item jsonb; failed boolean; total_n integer; done_n integer; state text;
begin
  ...
  update public.research_tasks set status=case when failed then 'failed' else 'completed' end,completed_at=now(),locked_at=null,locked_by=null,output=jsonb_build_object('summary',left(p_result->>'summary',3000)),error=case when failed then 'Research provider could not complete this task.' else null end where id=t.id;
