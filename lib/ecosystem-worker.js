...
function physicalApplication(text) {
  ...
  return /\b(robot(?:s|ic|ics)?|humanoid|locomotion|manipulation|embodied|self-driving|autonomous (?:vehicles?|drones?|robots?)|lidar|radar|sensors?|physical (?:systems|simulation|modelling|modeling|experiments)|physics|molecular|materials? (?:discovery|design)|laboratory|surgical|manufactur\w*|industrial control|agricultur\w*)\b/i.test(text)
    && /\b(build\w*|develop\w*|learn\w*|train\w*|control\w*|simulat\w*|model\w*|automat\w*|design\w*|deploy\w*|operat\w*|perceiv\w*|predict\w*)\b/i.test(text);
}
