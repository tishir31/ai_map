...
function assemble(base, tasks, run, now) {
  const dataset=structuredClone(base.discovery); const held=tasks.reduce((sum,t)=>sum+(t.result?.held?.length||0),0);
  const failed=tasks.filter(x=>x.status==="failed").length;
  const pending=tasks.filter(x=>["pending","running"].includes(x.status)).length;
  const plannedUrls=new Set(tasks.filter(x=>x.kind==='source').map(x=>x.input.source.url));
  const remainingBacklog=Array.isArray(run.source_backlog)?run.source_backlog.filter(url=>!plannedUrls.has(url)).length:(run.backlog||0);
  let changed=0;
  for(const task of tasks) {
    ...
    for(const item of result.accepted||[]) {
      ...
      dataset.changes=merge(dataset.changes,[{id:`ECO-CHANGE-${record.id}`,kind:recent?"event":"newly_mapped",summary:item.claim ? item.claim.text : `${item.relationship.predicate.replaceAll("_"," ")} relationship added from a public source`,entityIds:ids,communityIds:dataset.communities.filter(c=>[...c.personIds,...c.companyIds,...c.anchorWorkIds,...c.hubIds].some(id=>ids.includes(id))).map(c=>c.id),sourceIds:[task.source_id],eventDate,observedAt:result.checkedAt.slice(0,10),publishedAt:now}]);
    }
  }
...
async function runWeekly(config,{asOf}={},dependencies={}) {
  ...
  const results=await Promise.all([0,1].map(async()=>{
    ...
    try {
      ...
      await rpc(config,"finish_ecosystem_task",{p_task_id:task.id,p_lock:task.locked_by,p_result:result,p_status:"failed",p_retry:error.retryable===true});
      return {id:task.id,status:"failed",retryable:error.retryable===true};
    }
  }));
...
module.exports={runWeekly,planTasks,sourceTask,searchTask,stripHtml,modelJson,parseModelJson,assemble,finalize,publicSource};
