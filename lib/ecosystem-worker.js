...
const {ycCompanyIdentity,normalize:normalizeRegistryName}=require("./ecosystem-registry-parser");
...
function retryableError(error) { return error.retryable===true || ['TimeoutError','AbortError'].includes(error.name) || /operation was aborted due to timeout|^Source (?:read timed out|DNS timed out|deadline reached)\./i.test(error.message||''); }
async function modelJson(prompt, { search = false, fetchImpl = fetch } = {}) {
  ...
  const response = await fetchImpl(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(key)}`, {
    // 9s source read + 45s model call leaves checkpoint headroom in the 85s lease.
    method: "POST", headers: { "Content-Type": "application/json" }, signal: AbortSignal.timeout(45000),
    body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], ...(search ? { tools: [{ google_search: {} }] } : {}), generationConfig: { temperature: 0, maxOutputTokens: 7000, ...(/^gemini-2\.5-flash/.test(model)?{thinkingConfig:{thinkingBudget:0}}:{}), ...(!search ? { responseMimeType: "application/json" } : {}) } }),
  ...
  const diagnostics={model,finishReason:candidate?.finishReason||'missing',outputTokens:count(data.usageMetadata?.candidatesTokenCount),thoughtTokens:count(data.usageMetadata?.thoughtsTokenCount),totalTokens:count(data.usageMetadata?.totalTokenCount)};
  const context=`finish=${diagnostics.finishReason}; output=${diagnostics.outputTokens}; thoughts=${diagnostics.thoughtTokens}`;
  if(diagnostics.finishReason!=='STOP'){const error=Error(`Ecosystem model incomplete response (${context})`);error.modelDiagnostics=diagnostics;error.retryable=diagnostics.finishReason==='MAX_TOKENS';throw error;}
  let parsed;
  try {parsed=parseModelJson(candidate?.content?.parts?.filter(x => x.text&&!x.thought).map(x => x.text).join(""));}
  catch(error){error.message=`Ecosystem model JSON parse failed (${context}): ${error.message}`;error.modelDiagnostics=diagnostics;throw error;}
  return { data:parsed, groundedUrls: (candidate?.groundingMetadata?.groundingChunks || []).map(x => x.web?.uri).filter(Boolean), model, diagnostics };
}
...
function planTasks(run, dataset, registered) {
  ...
  const eligibleSources=[...sourceMap.values()].filter(source => run.cadence !== "daily" || source.verifiedIdentity === true || ["publication", "repository"].includes(source.kind));
  const deferredSources=eligibleSources.filter(source => new Date(byUrl.get(source.url)?.retry_after || 0).valueOf() > Date.now());
  const sources=eligibleSources.filter(source => !deferredSources.includes(source));
  const selected = sources.sort((a,b) => (byUrl.get(a.url)?.last_attempt_at || byUrl.get(a.url)?.last_checked_at || "").localeCompare(byUrl.get(b.url)?.last_attempt_at || byUrl.get(b.url)?.last_checked_at || ""));
  ...
  if (run.cadence !== "daily") for(const area of dataset.areas) tasks.push({id:`${run.id}-search-${area.id}`,run_id:run.id,kind:"search",source_id:`search-${area.id}`,area_id:area.id,input:{area}});
  const backlogUrls=[...ordered.slice(run.max_fetches),...deferredSources].map(source=>source.url);
  return {tasks,backlog:backlogUrls.length,backlogUrls};
}
async function searchTask(config, task, dependencies) {
  ...
  return {status: sources.length ? "checked" : "held",outcome:sources.length ? "checked" : "unresolved",kind:"search",sourceCount:sources.length,grounded:response.groundedUrls.length>0,model:response.model,modelDiagnostics:response.diagnostics,checkedAt:new Date().toISOString(),note:"Search results are candidates; each source must be fetched and checked separately."};
}
function physicalApplication(text) {
  // A street address, office, or generic use of "physical" is not an application.
  return /\b(robot(?:s|ic|ics)?|humanoid|locomotion|manipulation|embodied|self-driving|autonomous (?:vehicles?|drones?|robots?)|lidar|radar|sensors?|physical (?:systems|automation|simulation|modelling|modeling|experiments)|physics|molecul(?:e|es|ar)|materials? (?:discovery|design)|laboratory|surgical|manufactur\w*|industrial control|agricultur\w*)\b/i.test(text)
    && /\b(build\w*|develop\w*|learn\w*|train\w*|control\w*|simulat\w*|model\w*|automat\w*|design\w*|deploy\w*|operat\w*|perceiv\w*|predict\w*|mak(?:e|es|ing)|herd\w*)\b/i.test(text);
}
async function sourceForExecution(config,source) {
  const url=policy.canonicalUrl(source.url);
  const rows=await graph.restRequest(config,'ecosystem_sources',{params:{select:'url,active,descriptor,content_hash,last_outcome',url:`eq.${url}`,active:'eq.true',limit:1}});
  const row=rows[0];let registered=false;
  try {registered=row?.active===true&&row.url===url&&policy.canonicalUrl(row.descriptor?.url)===url;} catch { /* Invalid registry metadata never grants authority. */ }
  // Resolve authority at execution, including revocations, without rewriting the
  // frozen task, captured evidence, candidate outcomes, or its source identity.
  return {...source,...(registered?{kind:row.descriptor.kind,publisher:row.descriptor.publisher,contentHash:row.content_hash||source.contentHash,lastOutcome:row.last_outcome}:{}),verifiedIdentity:registered&&row.descriptor.verifiedIdentity===true};
}
async function resolveNewEntities(config, task, parsed, text, entities, html='',finalUrl=null) {
  ...
  for(const candidate of (parsed.newEntities||[]).slice(0,5)) {
    try {
      ...
      const companyRegistry=candidate.kind==='company'?ycCompanyIdentity(html,sourceUrl,finalUrl):null;
      const verifiedCompany=companyRegistry&&normalizeRegistryName(candidate.name)===companyRegistry.name;
      const authoritative=task.input.source.verifiedIdentity===true || registry || verifiedCompany;
      if(!authoritative || url!==sourceUrl || !["person","company","lab","project","software","paper","dataset","model","benchmark"].includes(candidate.kind) || !text.includes(policy.normalize(candidate.quote)) || !candidate.quote?.includes(candidate.name) || !candidate.physicalRelevance || candidate.physicalRelevance.length<12 || !text.includes(policy.normalize(candidate.physicalRelevance)) || !physicalApplication(candidate.physicalRelevance) || verifiedCompany&&!normalizeRegistryName(companyRegistry.applicationText).includes(normalizeRegistryName(candidate.physicalRelevance))) {held.push({name:candidate.name,reason:"new_identity_requires_official_capture"});continue;}
      const sameName=entities.filter(x=>[x.canonicalName,...(x.aliases||[])].some(name=>name.toLowerCase()===String(candidate.name).toLowerCase()));
      if(sameName.length || companyRegistry&&entities.some(x=>x.externalIds?.ycCompanyId===companyRegistry.registryId)) {held.push({name:candidate.name,reason:"possible_duplicate_identity"});continue;}
      ...
      const entity={id,kind:candidate.kind,canonicalName:candidate.name,displayName:candidate.name,aliases:[],externalIds:{officialProfile:url,...(verifiedCompany?{ycCompanyId:companyRegistry.registryId}:{})},visibility:"public"};
...
async function sourceTask(config, task, dataset, fullGraph, dependencies) {
  ...
  if(contentHash===source.contentHash&&source.lastOutcome!=='held') return {status:"unchanged",outcome:"checked",source:{...source,checkedAt,outcome:"checked",contentHash},sourceUrl:source.url,contentHash,checkedAt,accepted:[],held:[],entities:[]};
  ...
  let response;
  try {response=await dependencies.modelJson(extractionPrompt(source,text,fullGraph.entities));}
  catch(error) {error.capture={url:page.finalUrl||source.url,sha256:contentHash,text};error.retrieval={status:page.status,checkedAt,sourceUrl:source.url};throw error;}
  ...
  const resolved=await resolveNewEntities(config,task,response.data,text,entities,page.text,page.finalUrl);
  ...
  return {status:held.length ? "held" : "checked",outcome:"checked",source:{...source,checkedAt,outcome:"checked",contentHash,excerpt:quoteWords===0&&accepted.length ? policy.normalize(response.data.candidates[0]?.quote).split(/\s+/).slice(0,25).join(" ") : undefined},sourceUrl:source.url,contentHash,checkedAt,accepted,held,entities:resolved.additions,capture:{url:page.finalUrl,sha256:contentHash,text},model:response.model,modelDiagnostics:response.diagnostics};
}
...
async function runWeekly(config,{asOf,incremental=false}={},dependencies={}) {
  const deps={readPublicPage,modelJson,...dependencies};
  const run=await rpc(config,incremental ? "acquire_ecosystem_incremental_run" : "acquire_ecosystem_run",{p_batch_date:asOf});
  ...
  const results=await Promise.all([0,1].map(async()=>{
    ...
    try {
      const executingTask=task.kind==='source'?{...task,input:{...task.input,source:await sourceForExecution(config,task.input.source)}}:task;
      const result=task.kind==="source" ? await sourceTask(config,executingTask,base.discovery,fullGraph,deps) : await searchTask(config,task,deps);
      ...
      const result={outcome:error.sourceOutcome||"unresolved",error:String(error.message).slice(0,400),checkedAt:new Date().toISOString(),...(error.capture?{capture:error.capture,retrieval:error.retrieval}:{}),...(error.modelDiagnostics?{modelDiagnostics:error.modelDiagnostics}:{})};
...
module.exports={runWeekly,planTasks,sourceTask,searchTask,stripHtml,modelJson,parseModelJson,assemble,finalize,publicSource,quoteWordCount,claimsForSource,retryableError,physicalApplication,sourceForExecution};
