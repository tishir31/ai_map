...
async function sourceForExecution(config,source) {
  ...
  const rows=await graph.restRequest(config,'ecosystem_sources',{params:{select:'url,active,descriptor',url:`eq.${url}`,active:'eq.true',limit:1}});
  ...
  return {...source,...(registered?{kind:row.descriptor.kind,publisher:row.descriptor.publisher}:{}),verifiedIdentity:registered&&row.descriptor.verifiedIdentity===true};
}
...
async function runWeekly(config,{asOf}={},dependencies={}) {
  ...
  const results=await Promise.all([0,1].map(async()=>{
    ...
    try {
      ...
      const result={outcome:error.sourceOutcome||"unresolved",error:String(error.message).slice(0,400),checkedAt:new Date().toISOString(),...(error.capture?{capture:error.capture,retrieval:error.retrieval}:{})};
