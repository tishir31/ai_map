...
function validateDiscovery(discovery, existingIds = new Set(), existingEvidenceIds = new Set()) {
  ...
  const evIds = new Set([...existingEvidenceIds, ...discovery.extensions.evidence.map(x => x.id)]);
...
async function prepare(snapshot, { selectedVersion, publishedAt = new Date().toISOString(), coverage = { checked: 0, held: 0, failed: 0, remaining: 0 }, status = "published" } = {}) {
  ...
  for (const name of ["core", "research"]) {
    output[name].metadata.snapshotVersion = selectedVersion;
    // This labels the assembled snapshot, not the dates of its individual facts.
    output[name].metadata.asOf = output.discovery.researchAsOf;
    runtime.validateRuntimeTransport(output[name]);
  }
  const ids = new Set([...output.core.entities, ...output.research.entities].map(x => x[0]));
  validateDiscovery(output.discovery, ids, new Set([...output.core.evidence, ...output.research.evidence].map(x => x[0])));
