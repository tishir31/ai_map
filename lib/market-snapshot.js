...
const SNAPSHOT_READ_CAPS = Object.freeze({ activities: 1000, companies: 2000, exclusions: 2000 });
...
function buildSnapshot({ activities, companies, exclusions, ingestionRuns }, now = new Date()) {
  const rawInputs = { activities, companies, exclusions, ingestionRuns };
  const comparisonComplete = Object.entries(SNAPSHOT_READ_CAPS).every(([key, cap]) => Array.isArray(rawInputs[key]) && rawInputs[key].length < cap);
  const comparisonCoverage = comparisonComplete
    ? { complete: true }
    : { complete: false, reason: "one or more snapshot inputs reached a read cap or was not a successful array" };
