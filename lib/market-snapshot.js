...
function isPublicSafeActivity(row) {
  ...
  // Gmail-derived rows remain private even when an analyst later attaches a
  // public corroborating URL. A public publication must be authored as its own
  // non-Gmail activity instead of laundering private provenance through an
  // additional source.
...
async function loadSnapshot(config, now = new Date()) {
  const [activities, companies, exclusions, ingestionRuns] = await Promise.all([
    supabaseGet(config, "activities?select=id,date_announced,company_id,counterparty,activity_type,subsector,deal_value_usd,geography,description,source_id,source_url,source_reference,source_type,additional_sources,confidence,review_status,last_updated,is_sample,approved_at&review_status=eq.approved&is_sample=eq.false&order=date_announced.desc&limit=1000"),
