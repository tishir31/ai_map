...
function isPublicSafeActivity(row) {
  ...
  // additional source.
  const gmailDerived = String(row.source_type || "").trim().toLowerCase() === "gmail"
    || /^a-rq-gmail-/i.test(String(row.id || ""))
    || /gmail/i.test(String(row.entered_by || ""));
  if (gmailDerived) return false;
