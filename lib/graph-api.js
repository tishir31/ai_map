...
const PHYSICAL_AI_TERMS = /\b(?:physical ai|robot(?:ics)?|autonom(?:y|ous)|embodied|spatial intelligence|world model|simulation|perception|manipulation|mobility|ai for science)\b/i;
const PUBLIC_SIGNAL_MODEL_VERSION = "public-graph-v2";
const STARTUP_TERMINAL_ENDPOINTS = new Map([
  ["founded", "object"],
  ["spun_out_of", "subject"],
  ["launched_from", "subject"],
  ["commercializes", "subject"],
  ["released_by", "object"],
]);
...
function accessTokenUsesRecovery(token) {
  try {
...
async function verifyUser(req, config, { required = false } = {}) {
  ...
  // Supabase has already validated the token above. A recovery session is
  // sufficient only for changing the user's password; it must never unlock
  // private dossiers, signals, review work, or service-role-backed reads.
  if (accessTokenUsesRecovery(token)) {
    if (required) throw new ApiError(403, "Finish password recovery before using analyst features.");
    return null;
  }
...
async function fetchNeighborhood(config, rootId, hops, asOf) {
  ...
  for (let depth = 0; depth < hops && frontier.length; depth += 1) {
    if (frontier.length > 400) { frontier = frontier.slice(0, 400); truncated = true; }
    ...
    const batches = await Promise.all(groups.map((ids) => restRequest(config, "graph_current_relationships", {
      params: {
        select: "id,subject_entity_id,predicate,object_entity_id,layer,semantic_key,current_revision,started_on,started_precision,ended_on,ended_precision,observed_at,conclusion_label,review_status",
    ...
    if (batches.some((batch) => batch.length >= 2000)) truncated = true;
    const next = new Set();
...
function signalBand(value, medium, high) {
  if (value >= high) return "high";
  if (value >= medium) return "medium";
  return "low";
}

function publicPathInputIds(paths) {
  return unique((paths || []).flatMap((item) => item.path?.relationshipIds || []));
}

function publicIntelligencePath(entity, path) {
  return {
    entityId: entity.id,
    path,
    conclusionLabel: "Derived signal",
...
function isNearbyStartupRelationship(entity, relationship) {
  if (entity.kind !== "company" || !relationship) return false;
  const endpoint = STARTUP_TERMINAL_ENDPOINTS.get(relationship.predicate);
  return Boolean(endpoint && (endpoint === "subject"
    ? relationship.subject_entity_id
    : relationship.object_entity_id) === entity.id);
}

function isCurrentNetworkEmployerRelationship(entity, relationship, asOf) {
  return Boolean(entity.kind === "company"
    && relationship
    && relationship.predicate === "employed_by"
    && relationship.object_entity_id === entity.id
    && (!relationship.started_on || relationship.started_on <= asOf)
    && (!relationship.ended_on || relationship.ended_on > asOf));
}

// A single BFS path is insufficient when the same company has multiple
// equal-length terminal facts. Select a path per dossier section so a retained
// employment path cannot mask an equally short founding path (or vice versa).
function shortestPathWithTerminal(rootId, item, relationships, pathFor, qualifiesRelationship) {
  const targetId = item.entity.id;
  const expectedLength = item.path.steps.length;
  const terminalRelationships = relationships
    .filter((relationship) => (
      relationship.subject_entity_id === targetId || relationship.object_entity_id === targetId
    ) && qualifiesRelationship(relationship))
    .sort((left, right) => left.id.localeCompare(right.id));

  for (const relationship of terminalRelationships) {
    const otherId = relationship.subject_entity_id === targetId
      ? relationship.object_entity_id
      : relationship.subject_entity_id;
    if (otherId === targetId) continue;
    const prefix = pathFor(otherId);
    if (!prefix || prefix.entityIds.includes(targetId) || prefix.steps.length + 1 !== expectedLength) continue;
    const direction = relationship.subject_entity_id === otherId ? "forward" : "reverse";
    return {
      entityIds: [...prefix.entityIds, targetId],
      relationshipIds: [...prefix.relationshipIds, relationship.id],
      steps: [...prefix.steps, {
        ...
        to: targetId,
        direction,
        conclusionLabel: relationship.conclusion_label,
      }],
    };
  }
  return null;
...
function buildPublicIntelligence({
  rootId,
  asOf,
  entities,
  relationships,
  ...
  // Intelligence visible without authentication is rebuilt exclusively from
  // already-published entities, approved verified relationships, and public
  // evidence joins. Persisted analyst signals, candidate metrics, worklists,
  // review queues, warm paths, and owner-scoped coverage gaps are never read.
  const supportingEvidenceIds = new Set(evidenceBundle.evidence
    .filter((evidence) => evidence.stance === "support")
    .map((evidence) => evidence.id));
  const publicEvidenceRelationshipIds = new Set(evidenceBundle.relationshipEvidence
    .filter((link) => supportingEvidenceIds.has(link.evidence_id))
    .map((link) => link.relationship_id));
  const evidenceBackedRelationships = relationships.filter((relationship) =>
    relationship.conclusion_label === "Verified fact"
    && relationship.review_status === "approved"
    && publicEvidenceRelationshipIds.has(relationship.id));
  const entityById = indexById(entities);
  const relationshipById = indexById(evidenceBackedRelationships);
  const pathFor = (id) => shortestPath(rootId, id, evidenceBackedRelationships);
  const neighborhood = entities
    .filter((entity) => entity.id !== rootId)
    .map((entity) => ({ entity, path: pathFor(entity.id) }))
    .filter((item) => item.path)
    .sort((left, right) => left.path.steps.length - right.path.steps.length
      || left.entity.canonical_name.localeCompare(right.entity.canonical_name));

  const companyNeighborhood = neighborhood.filter((item) => item.entity.kind === "company");
  const nearbyStartups = companyNeighborhood
    .map((item) => ({
      ...item,
      path: shortestPathWithTerminal(
        rootId,
        item,
        evidenceBackedRelationships,
        pathFor,
        (relationship) => isNearbyStartupRelationship(item.entity, relationship),
      ),
    }))
    .filter((item) => item.path)
    .map((item) => ({
      ...publicIntelligencePath(item.entity, item.path),
      whyNearby: item.path.steps.map((step) => step.predicate.replaceAll("_", " ")).join(" → "),
    }));
  const currentNetworkEmployers = companyNeighborhood
    .map((item) => ({
      ...item,
      path: shortestPathWithTerminal(
        rootId,
        ...
        evidenceBackedRelationships,
        pathFor,
        (relationship) => isCurrentNetworkEmployerRelationship(item.entity, relationship, asOf),
      ),
    }))
    .filter((item) => item.path)
    .map((item) => ({
      ...publicIntelligencePath(item.entity, item.path),
      whyConnected: "Current employment destination — not startup-lineage evidence",
    }));

  const factsWithPublicEvidence = new Set(factEvidence.map((link) => link.fact_id));
  const publicFacts = facts.filter((fact) =>
    fact.conclusion_label === "Verified fact" && factsWithPublicEvidence.has(fact.id));
  ...
  const evidenceIdsByRelationship = new Map();
  for (const link of evidenceBundle.relationshipEvidence) {
    if (!evidenceIdsByRelationship.has(link.relationship_id)) evidenceIdsByRelationship.set(link.relationship_id, []);
    evidenceIdsByRelationship.get(link.relationship_id).push(link.evidence_id);
  }
  const officialRosterRelationshipFor = (personId) => evidenceBackedRelationships.find((relationship) => {
    ...
    return (evidenceIdsByRelationship.get(relationship.id) || []).some((evidenceId) => {
  ...
  const authoredPapersByPerson = new Map();
  for (const relationship of evidenceBackedRelationships) {
    if (relationship.predicate !== "authored") continue;
    const person = entityById.get(relationship.subject_entity_id);
    const paper = entityById.get(relationship.object_entity_id);
    if (person?.kind !== "person" || paper?.kind !== "paper") continue;
    if (!authoredPapersByPerson.has(person.id)) authoredPapersByPerson.set(person.id, []);
    authoredPapersByPerson.get(person.id).push({ paperId: paper.id, relationship });
  }
  const recentCutoff = monthsAgo(asOf, 24);
  const emergingResearchers = neighborhood
    .filter((item) => item.entity.kind === "person")
    .map((item) => {
      const personFacts = factsByEntity.get(item.entity.id) || [];
      const rosterRelationship = officialRosterRelationshipFor(item.entity.id);
      const rosterCounterpartyId = rosterRelationship
        ? (rosterRelationship.subject_entity_id === item.entity.id
          ? rosterRelationship.object_entity_id
          : rosterRelationship.subject_entity_id)
        : null;
      const rosterCounterparty = rosterCounterpartyId ? entityById.get(rosterCounterpartyId) : null;
      const subtypeStage = rosterRelationship
        && EMERGING_STAGES.has(String(item.entity.subtype || "").toLowerCase())
        ? item.entity.subtype
        : null;
      const careerStage = personFacts.find((fact) => fact.fact_type === "career_stage")?.display_value || subtypeStage;
      const currentRole = personFacts.find((fact) => fact.fact_type === "current_role")?.display_value
        || (rosterRelationship
          ? `${rosterRelationship.predicate.replaceAll("_", " ")} ${rosterCounterparty?.canonical_name || rosterCounterpartyId}`
          : null);
      const recentPaperEntityIds = unique((authoredPapersByPerson.get(item.entity.id) || [])
        .filter(({ paperId, relationship }) => {
          const publishedOn = (factsByEntity.get(paperId) || [])
            .find((fact) => fact.fact_type === "published_on")?.as_of_date;
          const date = publishedOn || relationship.started_on;
          return Boolean(date && date >= recentCutoff && date <= asOf);
        })
        .map(({ paperId }) => paperId));
      if (!careerStage || !currentRole
        || !EMERGING_STAGES.has(String(careerStage).toLowerCase())
        || !recentPaperEntityIds.length) return null;
      return {
        ...publicIntelligencePath(item.entity, item.path),
        careerStage,
        currentRole,
        recentPaperEntityIds,
        label: "Research lead — not a prediction of founder success",
      };
    })
    .filter(Boolean);

  const companyMetrics = publicFacts
    .filter((fact) => COMPANY_METRIC_FACT_TYPES.includes(fact.fact_type))
    .map((fact) => ({
      ...
      datePrecision: fact.date_precision,
      disclosureStatus: fact.disclosure_status,
      metricScope: fact.metric_scope,
      conclusionLabel: "Verified fact",
    }));

  const rootCollectionIds = new Set(collectionMemberships
    .filter((membership) => membership.entity_id === rootId)
    .map((membership) => membership.collection_id));
  const rootCollections = collections.filter((collection) => rootCollectionIds.has(collection.id));
  const themeCollections = rootCollections.filter((collection) =>
    PHYSICAL_AI_TERMS.test(`${collection.name || ""} ${collection.scope_statement || ""}`));
  const recentResearch = neighborhood.filter((item) => item.path.steps.some((step) => {
    const relationship = relationshipById.get(step.relationshipId);
    return relationship?.layer === "research"
      && relationship.started_on
      && relationship.started_on >= recentCutoff
      && relationship.started_on <= asOf;
  }));
  const labReach = neighborhood.filter((item) => item.entity.kind === "lab");
  const companyReach = neighborhood.filter((item) => item.entity.kind === "company");
  const peopleReach = neighborhood.filter((item) => item.entity.kind === "person");
  const root = entityById.get(rootId);
  const commercialDistance = root?.kind === "company"
    ? 0
    : nearbyStartups[0]?.path.steps.length ?? Number.POSITIVE_INFINITY;
  const evidenceCoverage = relationships.length
    ? Math.round((evidenceBackedRelationships.length / relationships.length) * 100)
    : 0;
  const evidenceIds = new Set(evidenceBundle.relationshipEvidence
    .filter((link) => publicEvidenceRelationshipIds.has(link.relationship_id)
      && supportingEvidenceIds.has(link.evidence_id))
    .map((link) => link.evidence_id));
  const relevantEvidence = evidenceBundle.evidence.filter((item) => evidenceIds.has(item.id));
  const freshEvidence = relevantEvidence.filter((item) => item.retrieved_at
    && item.retrieved_at.slice(0, 10) >= recentCutoff
    && item.retrieved_at.slice(0, 10) <= asOf);
  const evidenceFreshness = relevantEvidence.length
    ? Math.round((freshEvidence.length / relevantEvidence.length) * 100)
    : 0;
  const evidenceQuality = Math.min(evidenceCoverage, evidenceFreshness);

  const axes = {
    theme_fit: {
      band: signalBand(themeCollections.length, 1, 2),
      explanation: themeCollections.length
        ? `${themeCollections.length} published collection${themeCollections.length === 1 ? "" : "s"} connect this entity to the tracked thesis.`
        : "No published collection currently establishes a Physical-AI theme fit.",
    },
    research_momentum: {
      band: signalBand(recentResearch.length, 1, 3),
      explanation: `${recentResearch.length} nearby node${recentResearch.length === 1 ? "" : "s"} carry a public, evidence-backed research transition in the trailing 24 months.`,
    },
    commercialization: {
      band: commercialDistance <= 1 ? "high" : commercialDistance === 2 ? "medium" : "low",
      explanation: root?.kind === "company"
        ? "The selected entity is a company."
        : Number.isFinite(commercialDistance)
          ? `The nearest mapped company is ${commercialDistance} public, sourced step${commercialDistance === 1 ? "" : "s"} away.`
          : "No mapped company is reachable within two public, sourced steps.",
    },
    bridge_reach: {
      band: labReach.length > 0 && companyReach.length > 0
        ? "high"
        : labReach.length + companyReach.length > 0 ? "medium" : "low",
      explanation: `${labReach.length} lab node${labReach.length === 1 ? "" : "s"} and ${companyReach.length} company node${companyReach.length === 1 ? "" : "s"} are visible within two public, sourced steps.`,
    },
    talent_concentration: {
      band: signalBand(peopleReach.length, 2, 4),
      explanation: `${peopleReach.length} named person${peopleReach.length === 1 ? " is" : "s are"} reachable within two public, sourced steps.`,
    },
    evidence_quality: {
      band: evidenceQuality >= 85 ? "high" : evidenceQuality >= 70 ? "medium" : "low",
      explanation: `${evidenceCoverage}% of visible verified relationships have valid public evidence; ${evidenceFreshness}% of those public evidence records were retrieved in the trailing 24 months.`,
    },
  };
  const highAxes = Object.entries(axes).filter(([, value]) => value.band === "high").map(([key]) => key);
  const outcome = evidenceCoverage < 70
    ? "Coverage gap — unranked"
    : axes.theme_fit.band === "high" && axes.evidence_quality.band === "high"
      && highAxes.filter((axisKey) => axisKey !== "theme_fit" && axisKey !== "evidence_quality").length >= 2
      ? "Investigate now"
      : axes.theme_fit.band !== "low" ? "Watch" : "Low priority";

  const originPaths = neighborhood.filter((item) => item.path.steps.some((step) => ORIGIN_PREDICATES.has(step.predicate)));
  const researchDna = neighborhood.filter((item) => RESEARCH_KINDS.has(item.entity.kind));
  const gaps = [
    neighborhood.length === 0 ? "No public, evidence-backed relationship is visible within two steps." : null,
    originPaths.length === 0 ? "No public, sourced institutional or founder origin is visible." : null,
    researchDna.length === 0 ? "No public paper, idea, dataset, model, or project is linked." : null,
    nearbyStartups.length === 0 && root?.kind !== "company" ? "No company is connected by a public, sourced founding, spinout, launch, commercialization, or release path within two steps." : null,
    emergingResearchers.length === 0 ? "No researcher meets the public career-stage, current-role, and recent-work rule." : null,
    evidenceCoverage < 70 ? `Public evidence coverage is ${evidenceCoverage}%; ranking requires at least 70%.` : null,
    ...rootCollections
      .filter((collection) => collection.coverage_outcome
        && collection.coverage_outcome !== "Complete against declared source")
      .map((collection) => `${collection.name}: ${collection.coverage_outcome}.`),
  ].filter(Boolean);

  const axisPathInputs = {
    research_momentum: publicPathInputIds(recentResearch),
    commercialization: publicPathInputIds(nearbyStartups.slice(0, 3).map((item) => ({ path: item.path }))),
    bridge_reach: publicPathInputIds([...labReach, ...companyReach]),
    talent_concentration: publicPathInputIds(peopleReach),
    evidence_quality: evidenceBackedRelationships.map((relationship) => relationship.id),
  };
  const signalInputs = Object.entries(axisPathInputs).flatMap(([axisKey, relationshipIds]) =>
    unique(relationshipIds).slice(0, 12).map((relationshipId) => ({
      axis_key: axisKey,
      relationship_id: relationshipId,
    })));

  return {
    publiclyDerived: true,
    nearbyStartups,
    currentNetworkEmployers,
    emergingResearchers,
    companyMetrics,
    signal: {
      as_of_date: asOf,
      coverage_pct: evidenceCoverage,
      evidence_freshness_pct: evidenceFreshness,
      outcome,
      axes,
    ...
    signalModel: {
      ...
      name: "Public evidence triage",
      methodology: "Deterministic bands derived only from published verified relationships and valid public evidence.",
      minimum_coverage: 70,
    },
    signalInputs,
    gaps,
...
async function buildPrivateDossier(config, user, rootId, asOf, entities, relationships, facts, collections, evidenceBundle) {
  ...
  const pathFor = (id) => shortestPath(rootId, id, relationships);
  const companyPaths = entities
    .filter((entity) => entity.kind === "company" && entity.id !== rootId)
    .map((entity) => ({ entity, path: pathFor(entity.id) }))
    .filter((item) => item.path)
    .sort((a, b) => a.path.steps.length - b.path.steps.length || a.entity.canonical_name.localeCompare(b.entity.canonical_name));
  const nearbyStartups = companyPaths
    .map((item) => ({
      ...item,
      path: shortestPathWithTerminal(
        rootId,
  ...
  const currentNetworkEmployers = companyPaths
    .map((item) => ({
      ...item,
      path: shortestPathWithTerminal(
        rootId,
...
async function investigateSnapshot(config, { id, hops, asOf, snapshotVersion }) {
  const snapshots = require("./graph-snapshot");
  const full = await snapshots.snapshotGraph(await snapshots.load(config, snapshotVersion));
  const entities = full.entities.map(x => ({ id: x.id, typed_key: x.typedId, kind: x.kind, subtype: x.subtype, canonical_name: x.canonicalName, display_name: x.displayName, aliases: x.aliases, external_ids: x.externalIds, publication_status: "published" }));
  const allRelationships = full.relationships.filter(x => !x.derived && x.conclusionLabel !== "Analyst thesis").map(x => ({ id: x.id, subject_entity_id: x.subjectId, object_entity_id: x.objectId, predicate: x.predicate, layer: x.layer, started_on: x.temporal.startDate || (x.temporal.startMonth ? `${x.temporal.startMonth}-01` : x.temporal.startYear ? `${x.temporal.startYear}-01-01` : null), ended_on: x.temporal.endDate || (x.temporal.endMonth ? `${x.temporal.endMonth}-28` : x.temporal.endYear ? `${x.temporal.endYear}-12-31` : null), date_precision: x.temporal.precision, conclusion_label: "Verified fact", current_revision: x.revision, review_status: "approved" }));
  const entity = entities.find(x => x.id === id);
  if (!entity) throw new ApiError(404, "Entity is absent from this public snapshot.");
  const active = allRelationships.filter(x => relationshipActiveAt(x, asOf));
  const ids = new Set([id]); let frontier = [id]; let truncated = false;
  for (let i = 0; i < hops; i += 1) {
    const current = new Set(frontier); const next = new Set();
    for (const rel of active) if (current.has(rel.subject_entity_id) || current.has(rel.object_entity_id)) {
      for (const endpoint of [rel.subject_entity_id, rel.object_entity_id]) if (!ids.has(endpoint)) { if (ids.size < 400) { ids.add(endpoint); next.add(endpoint); } else truncated = true; }
    }
    frontier = [...next];
  }
  const nodes = entities.filter(x => ids.has(x.id));
  const relationships = active.filter(x => ids.has(x.subject_entity_id) && ids.has(x.object_entity_id)).slice(0, 2000);
  const relIds = new Set(relationships.map(x => x.id));
  const relationshipEvidence = full.relationships.filter(x => relIds.has(x.id)).flatMap(x => x.evidenceIds.map(evidence_id => ({ relationship_id: x.id, revision: x.revision, evidence_id })));
  const evIds = new Set(relationshipEvidence.map(x => x.evidence_id));
  const rawEvidence = full.evidence.filter(x => evIds.has(x.id));
  const evidence = rawEvidence.map(x => ({ id: x.id, source_id: x.id, snapshot_id: null, stance: x.stance, locator: x.locator, excerpt: null, directness: "direct", retrieved_at: x.retrievedAt }));
  const sources = rawEvidence.map(x => ({ id: x.id, canonical_url: x.url, title: x.publisher, publisher: x.publisher, source_type: "public_page", tier: x.tier, accessibility_status: "accessible", last_retrieved_at: x.retrievedAt }));
  const collections = full.collections.filter(x => x.entityIds.some(key => ids.has(key))).map(x => ({ id: x.id, name: x.title, scope_statement: x.summary, coverage_outcome: x.sourceUniverse.status, as_of_date: full.metadata.asOf }));
  const collectionMemberships = full.collections.flatMap(x => x.entityIds.filter(key => ids.has(key)).map(entity_id => ({ collection_id: x.id, entity_id })));
  const facts = (full.facts || []).filter(x => ids.has(x.entityId)).map(x => ({ id: x.id, entity_id: x.entityId, fact_type: x.factType, display_value: x.displayValue, numeric_value: x.numericValue, currency: x.currency, period_label: x.periodLabel, as_of_date: x.asOf, disclosure_status: x.disclosureStatus, conclusion_label: x.conclusionLabel, revision: x.revision }));
  const factEvidence = (full.facts || []).filter(x => ids.has(x.entityId)).flatMap(x => x.evidenceIds.map(evidence_id => ({ fact_id: x.id, evidence_id })));
  const evidenceBundle = { evidence, relationshipEvidence, sources, sourceSnapshots: [] };
  const classified = classifyPublicDossier(id, nodes, relationships);
  return { snapshotVersion, requestedEntity: null, entity, hops, asOf, truncated, limits: { maxFrontier: 400, relationshipsPerBatch: 2000 }, nodes, relationships, ...evidenceBundle, facts, factEvidence, collections, collectionMemberships, originPaths: classified.origin, researchDna: classified.researchDna, publicEvidenceSummary: evidenceSummary(relationships, relationshipEvidence, evidence, sources), intelligenceAccess: "public-read", privateAvailable: false, publicIntelligence: buildPublicIntelligence({ rootId: id, asOf, entities: nodes, relationships, facts, factEvidence, collections, collectionMemberships, evidenceBundle }) };
}

async function investigate(config, { id, hops = 2, asOf, user = null, snapshotVersion = null }) {
  if (snapshotVersion) return investigateSnapshot(config, { id, hops, asOf, snapshotVersion });
  ...
  const [evidenceBundle, factBundle, collectionBundle] = await Promise.all([
    ...
    fetchCollections(config, neighborhood.entities.map((item) => item.id), true),
  ]);
  const classified = classifyPublicDossier(enrichedRoot.id, neighborhood.entities, neighborhood.relationships);
  const publicIntelligence = buildPublicIntelligence({
    rootId: enrichedRoot.id,
    asOf,
  ...
  return {
    ...
    limits: { maxFrontier: 400, relationshipsPerBatch: 2000 },
    ...
    intelligenceAccess: "public-read",
    publicIntelligence,
...
async function compare(config, { left, right, hops = 2, asOf, user = null, snapshotVersion = null }) {
  const [leftDossier, rightDossier] = await Promise.all([
    investigate(config, { id: left, hops, asOf, user, snapshotVersion }),
    investigate(config, { id: right, hops, asOf, user, snapshotVersion }),
  ]);
...
async function acquireRefreshRun(config, { cadence, asOf, policy, scheduled }) {
  ...
  const result = await restRequest(config, "acquire_graph_refresh_run", {
    method: "POST", rpc: true,
    body: { p_cadence: cadence, p_as_of: asOf, p_policy_id: policy.id, p_mode: policy.mode, p_scheduled: scheduled },
  });
  if (!result?.run) throw new ApiError(502, "Refresh acquisition returned no run.");
  return { ...result, idempotencyKey };
}

async function runRefresh(config, { cadence = "manual", asOf, scheduled = false } = {}) {
  if (cadence === "daily") return require("./ecosystem-worker").runWeekly(config, { asOf: parseAsOf(asOf), incremental: true });
  if (cadence === "weekly") return require("./ecosystem-worker").runWeekly(config, { asOf: parseAsOf(asOf) });
