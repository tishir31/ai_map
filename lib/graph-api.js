...
const PUBLIC_SIGNAL_MODEL_VERSION = "public-graph-v1";
...
function buildPublicIntelligence({
  ...
  const nearbyStartups = neighborhood
    .filter((item) => {
      if (item.entity.kind !== "company") return false;
      const finalStep = item.path.steps[item.path.steps.length - 1];
      const finalRelationship = finalStep ? relationshipById.get(finalStep.relationshipId) : null;
      // Legacy entity typing can classify a venture firm as a company. The
      // subject of `invested_in` is the investor, never a nearby startup.
      return !(finalRelationship?.predicate === "invested_in"
        && finalRelationship.subject_entity_id === item.entity.id);
    })
    .map((item) => ({
  ...
  const gaps = [
    ...
    nearbyStartups.length === 0 && root?.kind !== "company" ? "No company is connected within two public, sourced steps." : null,
