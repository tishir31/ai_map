...
async function verifyCron(req, expectedJob) {
  if (expectedJob) {
    const authorization = await authorizeScheduledRequest(req, expectedJob);
    if (!authorization.ok) throw new ApiError(authorization.status, authorization.error);
    return authorization;
  }
  const expected = process.env.CRON_SECRET;
  if (!expected) throw new ApiError(503, "CRON_SECRET is not configured.");
  if (!constantTimeEqual(bearerToken(req), expected)) throw new ApiError(401, "Invalid scheduler credential.");
