...
async function processPublicResearch(runId) {
  ...
  try {
    ...
    const result=await runResearchAgent({id:task.id,runId:task.run_id,type:task.type,status:'running',agentName:task.agent_name,createdAt:task.created_at,input:{...task.input,minAcceptedCompanies:8,rowContext:existing.map(r=>({companyName:r.company_name,website:r.website})),publicMode:true}}, {liveProvider:'gemini',geminiApiKey:process.env.GEMINI_API_KEY,geminiModel:'gemini-2.5-flash-lite',allowMockFallback:false});
    ...
    console.error("Public research task failed", task.id, error instanceof Error ? error.message : "Unknown failure");
    const rateLimited = /HTTP 429/.test(error instanceof Error ? error.message : "");
    await databaseRpc('finish_public_research_task',{p_task_id:task.id,p_lock:task.locked_by,p_result:{failed:true,rateLimited,summary:rateLimited ? 'The model provider is temporarily rate-limited. This task remains queued and can resume after the cooldown.' : 'This research step could not complete. Other pending tasks remain available; findings already saved still need citation review.'}});
    const failure = Error(rateLimited ? 'The model provider is temporarily rate-limited. Your run remains queued. Try again in five minutes.' : 'Research step failed. Existing findings are preserved; refresh for details.');
    failure.status = rateLimited ? 429 : 502; throw failure;
  }
}
