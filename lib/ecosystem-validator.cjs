...
// Source SHA256 ebc70b0e64981c97ce824b1b02e1067b7271f41d801338a401d4be6fe0cb6af9
...
const datasetCheck = shape({
  ...
  communities: list(shape({ id: string, aliasIds: optional(strings), presentationOrder: optional(number), collectionId: string, areaId, title: string, problem: string, whyItMatters: string, approaches: strings, hubIds: strings, anchorWorkIds: strings, personIds: strings, companyIds: strings, sourceIds: strings, coverage: shape({ status: choice("publisher_selection", "complete_against_declared_source", "unresolved"), description: string, gaps: strings, checkedAt: date }) })),
...
function validateEcosystemDataset(value) {
  ...
  const dataset = value;
  const communityKeys = new Set(dataset.communities.map((community) => community.id));
  for (const community of dataset.communities) {
    if (community.presentationOrder !== void 0 && (!Number.isInteger(community.presentationOrder) || community.presentationOrder < 0)) fail(community.id, "invalid presentation order");
    for (const alias of community.aliasIds ?? []) {
      if (!alias || communityKeys.has(alias)) fail(community.id, "ambiguous community alias");
      communityKeys.add(alias);
    }
  }
