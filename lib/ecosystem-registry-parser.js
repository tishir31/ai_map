...
function ycCompanyIdentity(html,sourceUrl,finalUrl) {
 try {
  ...
  // Longer biographies mix current products with founders' former employers.
  // The registry's company tagline is the narrow application identity anchor.
  return {name:normalize(company.name),url:canonical,registryId:String(company.id),applicationText:normalize(company.one_liner)};
 } catch {return null;}
}
