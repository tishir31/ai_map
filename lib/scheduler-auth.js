...
function secretConfigurationError(secret, otherSecrets = [], label = "Scheduler credential") {
  ...
  if (Buffer.byteLength(secret) < MIN_SECRET_BYTES) return `${label} must be at least ${MIN_SECRET_BYTES} bytes.`;
...
async function readSchedulerSecretFromVault(env = process.env, { fetchImpl = globalThis.fetch } = {}) {
  ...
  try {
    ...
    secret = typeof payload === "string" ? payload.trim() : "";
  } catch {
    return unavailable();
  }
...
async function authorizeScopedSchedulerRequest(req, expectedJob, env = process.env, otherSecrets = [], options = {}) {
  ...
  if (!constantTimeEqual(bearerToken(req), resolved.secret)) return unauthorized();
  ...
  if (!validIsoDate(runDate) || runKey !== `${expectedJob}:${runDate}`) return unauthorized("Scheduler run key is invalid.");
  return {
    ok: true,
    status: 200,
    error: null,
