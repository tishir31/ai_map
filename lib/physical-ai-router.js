...
const scheduledHandlers = {
  "graph-refresh-daily": createScheduledRefreshHandler("daily"),
...
async function graphInvestigate(req, res) {
  ...
  try {
    ...
    const dossier = await graph.investigate(config, {
      ...
      user,
      snapshotVersion: graph.param(req, "version") || null,
    });
    return graph.sendJson(res, 200, { ok: true, dossier, ...(dossier.snapshotVersion ? { snapshotVersion: dossier.snapshotVersion } : {}) }, user
      ? "private, no-store"
      : "public, max-age=0, s-maxage=60, stale-while-revalidate=300");
  } catch (error) {
    return graph.sendError(res, error);
...
async function graphCompare(req, res) {
  ...
  try {
    ...
    return graph.sendJson(res, 200, { ok: true, comparison, ...(comparison.snapshotVersion ? { snapshotVersion: comparison.snapshotVersion } : {}) }, user
      ? "private, no-store"
      : "public, max-age=0, s-maxage=60, stale-while-revalidate=300");
  } catch (error) {
    return graph.sendError(res, error);
...
async function graphRefresh(req, res) {
  ...
  return graph.sendJson(res, 410, {
    ok: false,
    error: "This unscoped route is retired. Use the fixed weekly, monthly, or quarterly refresh route.",
  });
}
...
const routeHandlers = {
  "market-review-daily": require("./market-review-worker").handler,
  "market-snapshot": marketSnapshot,
  "graph-snapshot": require("./graph-snapshot").handler,
