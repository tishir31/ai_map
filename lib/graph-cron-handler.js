...
const SCHEDULED_CADENCES = new Set(["daily", "weekly", "monthly", "quarterly"]);
...
function createScheduledRefreshHandler(cadence, dependencies = graphApi) {
  ...
  const handler = async function scheduledGraphRefresh(req, res) {
    ...
    try {
      ...
      if (result.duplicate && !["completed", "published", "partial", "shadow", "checked"].includes(result.status)) {
        throw new graphApi.ApiError(
          503,
          `The existing ${cadence} graph refresh is ${result.status || "not complete"}; retry was not acknowledged.`,
        );
      }
      return dependencies.sendJson(res, result.stagingOnly || result.status === "running" ? 202 : 200, { ok: true, cadence, asOf, result });
    } catch (error) {
