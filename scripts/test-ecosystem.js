...
const {ycCompanyIdentity}=require('../lib/ecosystem-registry-parser');
...
async function main(){
 const authorityRest=graph.restRequest;const frozenSource={...source,url:'https://robotics.edu/people/',verifiedIdentity:true};
 try {
  graph.restRequest=async(_config,table,{params})=>{assert.equal(table,'ecosystem_sources');assert.equal(params.url,'eq.https://robotics.edu/people');assert.equal(params.active,'eq.true');return [{url:'https://robotics.edu/people',active:true,descriptor:{...source,kind:'institution',verifiedIdentity:true}}];};
  assert.equal((await worker.sourceForExecution({}, {...frozenSource,verifiedIdentity:undefined})).verifiedIdentity,true,'An old frozen slash-variant task can use exact reviewed registry authority');
  graph.restRequest=async()=>[];assert.equal((await worker.sourceForExecution({},frozenSource)).verifiedIdentity,false,'Removed or inactive registry entry revokes cached task authority');
  graph.restRequest=async()=>[{url:'https://robotics.edu/people',active:true,descriptor:{...source,verifiedIdentity:false}}];assert.equal((await worker.sourceForExecution({},frozenSource)).verifiedIdentity,false,'Explicit revocation overrides frozen true flag');
  graph.restRequest=async()=>[{url:'https://robotics.edu/people',active:false,descriptor:{...source,verifiedIdentity:true}}];assert.equal((await worker.sourceForExecution({},frozenSource)).verifiedIdentity,false);
  graph.restRequest=async()=>[{url:'https://robotics.edu/people',active:true,descriptor:{...source,url:'https://robotics.edu/other',verifiedIdentity:true}}];assert.equal((await worker.sourceForExecution({},frozenSource)).verifiedIdentity,false,'Other pages on the same host cannot authorize this source');
  assert.equal(frozenSource.verifiedIdentity,true,'Task input history is not mutated');assert.equal(frozenSource.url,'https://robotics.edu/people/');
 }finally{graph.restRequest=authorityRest;}
 const registryUrl='https://www.ycombinator.com/companies/b12-labs';
 const registryApplication='b12 Labs develops robot control software for industrial manufacturing.';
 const registryPage=(name='b12 Labs',application=registryApplication)=>{const title=`${name}: ${application} | Y Combinator`,page={component:'ycdc_new/pages/Companies/ShowPage',props:{signedIn:false,currentUser:null,company:{id:30689,name,slug:'b12-labs',one_liner:application,long_description:application,ycdc_url:registryUrl}}};const esc=s=>s.replaceAll('&','&amp;').replaceAll('"','&quot;');return `<title>${title}</title><link rel="canonical" href="${registryUrl}"><meta property="og:url" content="${registryUrl}"><meta property="og:title" content="${title}"><div data-page="${esc(JSON.stringify(page))}"><h1>${name}</h1><p>${application}</p></div>`;};
 const registryHtml=registryPage();assert.equal(ycCompanyIdentity(registryHtml,registryUrl,registryUrl).registryId,'30689');
 assert.equal(ycCompanyIdentity(registryHtml,registryUrl,'https://elsewhere.org/copied-yc-page'),null,'Cross-domain redirects cannot supply cloned registry identity');
 assert.equal(ycCompanyIdentity(registryHtml,'https://www.ycombinator.com/companies/b12-labs?user=data',registryUrl),null);
 ...
 try {
  const extract=async(html,candidate,known=entities)=>worker.sourceTask({}, {input:{source:registrySource},run_id:'yc-test'}, {}, {entities:known,relationships:[]}, {readPublicPage:async()=>({status:200,finalUrl:registryUrl,text:html}),modelJson:async()=>({data:{newEntities:[candidate],candidates:[]},model:'fixture'})});
  const created=await extract(registryHtml,registryCandidate);assert.equal(created.entities.length,1,'Structured official company registry can establish new company identity');assert.equal(created.entities[0].externalIds.ycCompanyId,'30689');assert.equal(created.source.verifiedIdentity,false,'Identity creation does not grant relationship authority to a registry source');
  const generic={name:'GenericAI',kind:'company',url:registryUrl,quote:'GenericAI',physicalRelevance:'GenericAI develops LLM tools from a physical office at 100 Main Street.'};assert.equal((await extract(registryPage(generic.name,generic.physicalRelevance),generic)).entities.length,0,'Physical office/address is not a physical application');
  assert.equal((await extract(registryHtml,{...registryCandidate,name:'Different Company'})).entities.length,0,'Exact registry name must agree');
  assert.equal((await extract(registryHtml,registryCandidate,[...entities,{id:'ENT-9999',kind:'company',canonicalName:'Previous company name',externalIds:{ycCompanyId:'30689'}}])).entities.length,0,'Stable registry ID prevents renamed duplicates');
  assert.equal(registryResolutions,1,'Only the verified physical company reaches atomic identity resolution');
 }finally{graph.restRequest=savedRegistryRest;}
 const timeout=Object.assign(new Error('The operation was aborted due to timeout'),{name:'TimeoutError'});assert.equal(worker.retryableError(timeout),true);assert.equal(worker.retryableError(new Error('Malformed extraction response')),false);
 assert.equal(worker.retryableError(new Error('Source read timed out.')),true);assert.equal(worker.retryableError(new Error('Source redirect or time limit reached.')),false,'Redirect loops must not become transient retries');
 const savedKey=process.env.GEMINI_API_KEY;process.env.GEMINI_API_KEY='fixture-not-a-real-key';try{await assert.rejects(worker.modelJson('fixture',{fetchImpl:async()=>{throw timeout;}}),error=>error.retryable===true);}finally{if(savedKey===undefined)delete process.env.GEMINI_API_KEY;else process.env.GEMINI_API_KEY=savedKey;}
 const queueDataset={areas:[{id:'robots'}],communities:[{areaId:'robots',sourceIds:['ONE','TWO']}],sources:[{id:'ONE',url:'https://example.org/one/'},{id:'TWO',url:'https://example.org/two/'}]};
 const queueRegistry=[{url:'https://example.org/one',descriptor:{id:'ONE',url:'https://example.org/one'},content_hash:'one-hash',last_checked_at:'2026-09-09'},{url:'https://example.org/two',descriptor:{id:'TWO',url:'https://example.org/two'},content_hash:'two-hash',last_checked_at:'2026-09-10'}];
 const planned=worker.planTasks({id:'queue-test',max_fetches:1},queueDataset,queueRegistry);assert.equal(planned.tasks.filter(t=>t.kind==='source').length,1);assert.equal(planned.backlog,1,'Canonical trailing slash variants count once');assert.deepEqual(planned.backlogUrls,['https://example.org/two']);assert.equal(planned.tasks[0].input.source.contentHash,'one-hash');
 ...
 await assert.rejects(worker.sourceTask({}, {input:{source},run_id:'timeout-capture'}, {}, {entities,relationships:[]}, {readPublicPage:async()=>({status:200,finalUrl:source.url,text:result.capture.text}),modelJson:async()=>{throw Object.assign(new Error('Provider timeout'),{name:'TimeoutError'});}}),error=>error.capture?.sha256===result.contentHash&&error.retrieval?.status===200,'A failed extraction retains the real private retrieval and hash without claiming completed research');
 ...
 const oldPlanAssembly=worker.assemble({discovery},[{id:'done',kind:'source',status:'checked',input:{source:{...source,url:'https://example.org/one/'}}}],{source_backlog:['https://example.org/one','https://example.org/two/','https://example.org/two']},'2026-09-10T00:00:00Z');assert.equal(oldPlanAssembly.coverage.remaining,1,'An older frozen queue is counted by unique canonical unprocessed URLs');
