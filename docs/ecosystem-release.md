...
For SQL connectors with a request-size limit, apply `20260910074154_physical_ai_ecosystem_bootstrap_upload.sql`, then generate `node scripts/bootstrap-ecosystem.js /path/to/ecosystem.v1.json --chunk-dir /path/to/empty-directory`. Execute numbered SQL files in order: begin, 48 KB binary chunks, finish. Each SQL request is below 65 KB; captured JSON is base64 data, never executable SQL. Private RLS-protected staging retains partial uploads for one day; identical chunk retries are safe, conflicts fail, and finalization verifies the exact envelope byte count and SHA256 before registering sources/identifiers and calling the existing atomic publication RPC. It clears chunks after success and retains a small idempotency receipt. Incomplete/corrupt uploads and publication conflicts leave the active snapshot and registrations untouched. The upload does not change research policy mode. Test the exact generated artifact in isolated PostgreSQL by setting `ECOSYSTEM_BOOTSTRAP_UPLOAD_DIR` alongside `ECOSYSTEM_PGLITE_PATH` when running the SQL test.
...
New companies can also resolve from an anonymous official YC company profile whose requested/final/canonical/structured registry URLs agree, with a positive registry ID and exact structured company name/title. Application evidence must occur in the company's registry tagline and describe work on physical systems; a physical office/address is insufficient. Longer biographies can mix products with founder history and are not accepted as this identity anchor. The YC registry ID and canonical URL are retained for duplicate checks. This establishes company identity only: it does not authorize founder, mentor, employment, funding or deployment edges, and unknown standalone domains remain held. The parser was checked against the public [b12 Labs profile](https://www.ycombinator.com/companies/b12-labs).

Existing curated claim summaries are conservatively counted against the 25-word source allowance because legacy records do not distinguish summaries from verbatim excerpts. This can hold additional claims from a source whose allowance is already used; it does not block other eligible sources or relationships.
...
Gemini 2.5 Flash extraction uses `thinkingBudget: 0`, matching the existing research adapter. Private outcomes retain completion reason and token counts; incomplete responses are rejected before JSON parsing. Previously held evidence is reconsidered even if its content hash has not changed, so corrected authority or newly resolved identities can be evaluated in a later batch. Prior task outcomes remain unchanged.
