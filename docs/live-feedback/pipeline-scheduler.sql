-- Apply after pipeline-telemetry and reviewed API deployment. Keep daily bounded to 30 sources,
-- no broad search, unchanged content hashes reuse the existing evidence/identity gates.
begin;
alter table private.physical_ai_scheduler_dispatches drop constraint physical_ai_scheduler_dispatches_job_name_check;
alter table private.physical_ai_scheduler_dispatches add constraint physical_ai_scheduler_dispatches_job_name_check check(job_name in ('ingest-gmail','ingest-web-news','graph-refresh-daily','graph-refresh-weekly','graph-refresh-monthly','graph-refresh-quarterly'));
...
-- Monday keeps the broad weekly batch. Other days check existing verified official/publication/repository sources.
select cron.schedule('physical-ai-graph-refresh-daily','0,30 15,16 * * 0,2-6',
  $job$select * from private.dispatch_physical_ai_job('graph-refresh-daily');$job$);
commit;
